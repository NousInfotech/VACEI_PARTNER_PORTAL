...
/** Section menu labels for Completion Procedures — match REFERENCE-PORTAL exactly */
const COMPLETION_SECTION_TITLES: Record<string, string> = {
...
const DEFAULT_COMPLETION_SECTIONS = Object.entries(COMPLETION_SECTION_TITLES).map(([sectionId, title]) => ({
  id: sectionId,
  sectionId,
  title,
  fields: [],
...
interface CompletionProcedureViewProps {
  procedure: any;
  engagement?: any;
  onRegenerate?: () => void;
...
export const CompletionProcedureView: React.FC<CompletionProcedureViewProps> = ({
  procedure: initial,
  engagement,
  ...
  const handleGenerateProcedures = async (sectionId: string) => {
    ...
    try {
      ...
      const data = await apiPost<any>(
        endPoints.COMPLETION_PROCEDURES.GENERATE_RECOMMENDATIONS(
          engagementId
        ),
        {
          procedures: [
  ...
  const handleExportCompletionPDF = async () => {
    try {
      ...
      const addFooter = () => {
        ...
        doc.text("Confidential — For audit completion purposes only.", margin, footerY);
      ...
      Array.isArray(proc.procedures) &&
        proc.procedures.forEach((sec: any, index: number) => {
          ...
          doc.setFont("helvetica", "bold");
          doc.setFontSize(14);
          doc.text(`Section: ${COMPLETION_SECTION_TITLES[sec.sectionId || sec.id] ?? sec.title ?? `Section ${index + 1}`}`, margin, 20);
          const body: any[] = [];
  ...
  return (
    <div className="space-y-6">
      <div className="text-sm text-muted-foreground font-body mb-4">
        Step-1: Generate questions for each completion section separately. You can freely edit / add / remove questions here before moving to the next step.
      </div>
      ...
      {sectionsToRender.length > 0 ? (
        ...
        >
          {sectionsToRender.map((sec: any, sIdx: number) => {
            ...
            const filteredReviewsForSection = reviews.filter(
              (r: any) =>
            ...
            const reviewsListContent = isLoadingReviews ? (
              <Card>
            ...
            ) : filteredReviewsForSection.length === 0 ? (
              <Card>
                <CardContent className="pt-6">
                  <div className="text-center text-muted-foreground py-8">
              ...
              <>
                {filteredReviewsForSection.map((review: any, rIdx: number) => {
                ...
                <AccordionTrigger className="hover:no-underline">
                  <div className="text-left">
                    <div className="font-heading text-lg">
                      {COMPLETION_SECTION_TITLES[sec.sectionId || sec.id] ?? sec.title ?? sec.sectionId ?? sec.id}
                    </div>
                                  ...
                                  onClick={async () => {
                                    ...
                                    try {
                                      ...
                                      await apiPostFormData(
                                        endPoints.COMPLETION_PROCEDURES.SAVE(
                                          engagementId
                                        ),
                                        form
                                      );
                            ...
                            <div className="space-y-6">
                              <div className="flex items-center justify-between mb-4">
                                <h5 className="text-md font-semibold flex items-center gap-2">
                                  ...
                                  Reviews (
                                  {filteredReviewsForSection.length}{" "}
                                  reviews)
                                </h5>
                                <Button
                                  variant="outline"
          ...
          <Button
            ...
            onClick={async () => {
              try {
                ...
                onWizardComplete({ ...payload, procedureType: "completion" });
              } catch (e: any) {
                toast({
                  title: "Save failed",
