...
export const CompletionRecommendationsStep: React.FC<CompletionRecommendationsStepProps> = ({
  ...
  const generateAI = async () => {
    ...
    try {
      const res = await apiPost<{ data?: { recommendations?: any[] }; recommendations?: any[] }>(
        endPoints.COMPLETION_PROCEDURES.GENERATE_RECOMMENDATIONS(engagementId),
        {
  ...
  const handleSaveAndComplete = async () => {
    ...
    try {
      ...
      toast({ title: "Saved", description: "Completion procedures and recommendations saved." });
      onComplete({ ...payload, procedureType: "completion" });
    } catch (e: any) {
      toast({
        title: "Save failed",
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-lg font-semibold text-gray-900 mb-1">Completion recommendations</h3>
