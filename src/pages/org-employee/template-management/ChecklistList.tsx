...
const ChecklistList: React.FC = () => {
  ...
  const queryParams: Record<string, string> = { type: 'CHECKLIST', limit: '100' };
  if (organizationMember?.organizationId) {
    queryParams.organizationId = organizationMember.organizationId;
  }

  const { data: response, isLoading } = useQuery({
    queryKey: ['templates', 'CHECKLIST', organizationMember?.organizationId],
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-3">
        ...
        <Button variant="header" onClick={() => navigate('/dashboard/templates/create?type=CHECKLIST&moduleType=ENGAGEMENT')}>
