...
const MilestoneList: React.FC = () => {
  ...
  const queryParams: Record<string, string> = { type: 'MILESTONES', limit: '100' };
  if (organizationMember?.organizationId) {
    queryParams.organizationId = organizationMember.organizationId;
  }

  const { data: response, isLoading } = useQuery({
    queryKey: ['templates', 'MILESTONES', organizationMember?.organizationId],
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-3">
        ...
        <Button variant="header" onClick={() => navigate('/dashboard/templates/create?type=MILESTONES&moduleType=ENGAGEMENT')}>
                  ...
                  <TableRow key={template.id} className="hover:bg-gray-50/50 transition-colors group">
                    ...
                    <TableCell className="py-4">
                      <div className="flex items-start gap-3">
                        <div className="mt-0.5 p-2 rounded-xl bg-amber-50 text-amber-600 shrink-0">
              ...
              <TableRow>
                <TableCell colSpan={5} className="py-24 text-center">
                  <div className="flex flex-col items-center gap-3 text-gray-400">
                    ...
                    <p className="font-medium">No milestone templates found</p>
                    <p className="text-xs">Create your first milestone template</p>
                  </div>
                </TableCell>
              </TableRow>
            )}
