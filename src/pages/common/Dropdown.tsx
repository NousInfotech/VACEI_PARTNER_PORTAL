...
export const Dropdown = ({
  ...
  }, [isOpen, searchable, align, side, fullWidth, autoPosition, children]);

  const filteredItems = useMemo(() => {
    if (!searchable || !searchQuery) return items;
    return items.filter((item) => {
      if (typeof item.label !== "string") return true; // Keep non-string labels
      return item.label.toLowerCase().includes(searchQuery.toLowerCase());
    });
  }, [items, searchQuery, searchable]);

  const highlightMatch = (text: React.ReactNode) => {
